...
void MemoryViewer::paintEvent(QPaintEvent *) {
    ...
    for (int row = 0; row < nRowsVisible; row++) {
        x = lx - offsetX;
        for (int col = 0; (col < mBytesPerLine) && (bPos < dataHex.size()); col++, bPos++) {
    ...
    for (int row = 0; row < nRowsVisible; row++) {
        ...
        for (int col = 0; (col < mBytesPerLine) && (bPos < dataVisible.size());
             col++) {
            ch = static_cast<uchar>(dataVisible.at(bPos));
            if (ch < 0x20 || ch > 0x7f)
                ch = '.';
...
void MemoryViewer::adjustContent() {
    ...
    startPos = static_cast<qint64>(val) * mBytesPerLine;
    endPos = startPos + nRowsVisible * mBytesPerLine - 1;

    int lineCount = static_cast<int>(size / mBytesPerLine);
